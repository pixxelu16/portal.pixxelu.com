      ...
      <div class="col-sm-6 col-md-3">
         <div class="input-block mb-3 form-focus select-focus">
            <select class="select floating" name="month">
               ...
               @foreach ($months as $m)
               <option value="{{ $m }}" {{ request()->input('month') == $m ? 'selected="selected"' : '' }}>{{ $m }}</option>
               @endforeach
            </select>
         </div>
      </div>
      <div class="col-sm-6 col-md-3">
         <div class="input-block mb-3 form-focus select-focus">
            <select class="select floating" name="year">
               ...
               <option value="{{ $i }}" {{ request()->input('year') == $i ? 'selected="selected"' : '' }}>{{ $i }}</option>
               @endfor
            </select>
         </div>
      </div>
      ...
      <div class="table-responsive">
         <table class="table table-striped custom-table table-nowrap mb-0">
            ...
            <tbody>
               ...
               $count = 1;
               @endphp
               @forelse ($get_employees_detail as $employee)
               <tr>
                  <td>{{ $count++ }}.</td>
